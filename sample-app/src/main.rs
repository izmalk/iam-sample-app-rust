...
fn main() {
    ...
    if databases
        .contains(DB_NAME)
        .expect("Failed to check existence of the database.")
    {
        println!("Found a pre-existing database! Re-creating with the default schema and data...");
        let _ = databases
            .get(DB_NAME)
            .expect("Failed to get the database for deletion.")
            .delete();
    }
    let _ = databases.create(DB_NAME);
    if databases
        .contains(DB_NAME)
        .expect("Failed to check creation of a database.")
    {
        println!("Empty database created.");
    }
    let db = databases
        .get(DB_NAME)
        .expect("Failed to retrieve the database to open a session.");
    ...
    let tx = session
        ...
        .expect("Failed to open a transaction.");
    let data =
        fs::read_to_string("iam-data-single-query.tql").expect("Failed to read a schema file.");
    let _ = tx
        .query()
        .insert(&data)
    ...
    let tx2 = session
        ...
        .expect("Failed to open a transaction."); //Re-using a same session to open a new transaction
    let read_query = "match $u isa user; get $u; count;";
    let count = tx2
        .query()
        .get_aggregate(&read_query)
        .resolve()
    ...
    let iterator = tx
        .query()
        .get(typeql_read_query)
        .expect("Failed to read data."); //Executing the query
    let mut k = 0; // Counter
    ...
    let tx = session
        .transaction_with_options(TransactionType::Read, Options::new().infer(true))
        .expect("Failed to open a transaction."); //Inference enabled
    let typeql_read_query = "match 
            $u isa user, has full-name 'Kevin Morrison'; 
            $p($u, $pa) isa permission; 
            $o isa object, has path $fp; 
    ...
    let tx = session
        .transaction(TransactionType::Write)
        .expect("Failed to open a transaction."); //Open a transaction to write
    let timestamp = Utc::now().format("%Y-%m-%d-%H-%M-%S").to_string();
    let filename = format!("{}{}{}", "logs/", timestamp, ".log");
    let typeql_insert_query = format!("insert $f isa file, has path '{}';", filename);
    let _query_response: Vec<Result<typedb_driver::answer::ConceptMap, typedb_driver::Error>> = tx
        .query()
        .insert(&typeql_insert_query)
        .expect("Failed to read data.")
        .collect(); //Inserting file
    ...
    let _query_response: Vec<Result<typedb_driver::answer::ConceptMap, typedb_driver::Error>> = tx
        ...
        .insert(&typeql_insert_query)
        .expect("Failed to read data.")
        .collect(); //The second query in the same transaction
